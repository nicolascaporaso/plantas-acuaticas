const misProductos = "../plantas.json";
let productos = [];
...
const mostrarProductos = (items) => {
    ...
    items.forEach((item) => {
        ...
        <p class="tienda__grid__card__parrafo">${item.descripcionCatalogo || ""}</p>
        <a class="tienda__grid__card__comprar comprar" href="${item.tiendaUrl || "#"}" aria-label="Comprar ${item.nombre}">Comprar</a>
        <div class="position">
            <p class="tienda__grid__card__parrafo cantidadoculta-carrito posicion" id="compro${item.id}">cantidad: </p>
            <a class="tienda__grid__card__link" href="${item.ficha.ruta}" target="_blank" rel="noopener noreferrer" aria-label="Abrir ficha técnica de ${item.nombre}">
                ...
                <img class="tienda__grid__card__img" src="${item.imagen}" alt="${item.altText || item.nombre}">
            </a>
        </div>`;
